...
def create_seq_features(vocab_file, vocabulary_size, remove_stopwords,max_seq_length):
    ...
    x_train = sequence.pad_sequences(train_data, maxlen=max_seq_length)
    x_val = sequence.pad_sequences(val_data, maxlen=max_seq_length)
    x_test = sequence.pad_sequences(test_data, maxlen=max_seq_length)
    y_train = df_train_targets.values
    y_val = df_validate_targets.values
...
def main():
    ...
    # X_train, y_train, X_val, y_val, _, _ = create_noseq_features(vocabulary_size=5000, load_vocab_file=False, remove_stopwords=True)
    ...
    X_train, y_train, X_val, y_val = create_seq_features(vocab_file=vocab_file, vocabulary_size=5000, max_seq_length=100,
                                                                 remove_stopwords=True)
    save_features_labels(X_train, y_train, "experiments/data/x_train_noseq.pickle",
                         "experiments/data/y_train_seq.pickle")
    save_features_labels(X_val, y_val, "experiments/data/x_val_seq.pickle", "experiments/data/y_val_seq.pickle")
    pass
